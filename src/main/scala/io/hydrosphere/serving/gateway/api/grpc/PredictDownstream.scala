...
object PredictDownstream {
  ...
  def mkStubAndChannel(service: StoredService, deadline: Duration): WrappedStub = {
    mkStubAndChannel(service.host, service.port, service.modelVersion.id, deadline)
  }

  def mkStubAndChannel(host: String, port: Int, modelVersionId: Long, deadline: Duration): WrappedStub = {
    ...
    new WrappedStub(stub, deadline, channel, modelVersionId)
  }
  ...
  class BalancedDownstream(
    distributor: Distributor[WrappedStub]
  ) extends Actor {
  ...
  object BalancedDownstream {
    ...
    def props(distributor: Distributor[WrappedStub]): Props = Props(classOf[BalancedDownstream], distributor)
  }
