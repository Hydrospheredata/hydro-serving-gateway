...
import io.hydrosphere.serving.proto.contract.tensor.definitions.{Int64Tensor, StringTensor}
...
class ExecutionTests extends GenericTest {
  describe("Executors") {
    ...
    it("single servable without shadow") {
        ...
        override def make(host: String, port: Int): IO[PredictionClient[IO]] = IO {
          new PredictionClient[IO] {
            override def predict(request: PredictRequest): IO[PredictResponse] = IO {
              PredictResponse(request.inputs + ("dummy" -> StringTensor(Shape.scalar, Seq("hello")).toProto))
            }
      ...
      val req: GatewayPredictRequest = GatewayPredictRequest(
        name = "test",
        data = Map(
          "test" -> StringTensor(Shape.scalar, Seq("hello")).toProto
        )
      )
      val response = executionService.predictShadowlessServable(req).unsafeRunSync()
    ...
    it("shoud fail execution if output violates the contract") {
      ...
      val shadow: MonitoringClient[IO] = (req: ServableRequest, resp: AssociatedResponse, appInfo: Option[ApplicationInfo]) => {
        val entry = MonitoringClient.mkExecutionMetadata(mv, appInfo, resp.resp.latency, req.requestId)
        shadowState += entry
        IO(entry)
      }
    ...
    it("Servable with shadowing execution") {
      ...
      val response = executionService.predictServable(req).unsafeRunSync()

      val data = response.outputs
      assert(data.contains("dummy"))
      assert(shadowState.length == 1)
      assert(shadowState.head._1.data === req.data)
      assert(shadowState.head._2.servable === servable)
      assert(shadowState.head._2.resp.data == Map(
        "dummy" -> StringTensor(Shape.scalar, Seq("hello")).toProto,
        "test" -> StringTensor(Shape.scalar, Seq("hello")).toProto
      ).asRight)
        ...
        IO {
          _: ServableRequest =>
            IO {
              ServableResponse(
                data = Map(
                  "model" -> StringTensor(Shape.scalar, Seq(x.modelVersion.name)).toProto,
                  "version" -> Int64Tensor(Shape.scalar, Seq(x.modelVersion.version)).toProto
                ).asRight,
                latency = 0
              )
            }
    ...
    it("Pipeline executor success route") {
      val e1: Predictor[IO] = (request: ServableRequest) => IO {
        ...
        val toInject = Map(
          "exec1" -> StringTensor(Shape.scalar, Seq("exec1")).toProto
        )
      ...
      val e2: Predictor[IO] = (request: ServableRequest) => IO {
        ...
        val toInject = Map(
          "exec2" -> StringTensor(Shape.scalar, Seq("exec2")).toProto
        )
        val resp = ServableResponse(
          data = (request.data ++ toInject).asRight,
          latency = 2
      ...
      val e3: Predictor[IO] = (request: ServableRequest) => IO {
        ...
        val toInject = Map(
          "exec3" -> StringTensor(Shape.scalar, Seq("exec3")).toProto
        )
        val resp = ServableResponse(
          data = (request.data ++ toInject).asRight,
          latency = 3
